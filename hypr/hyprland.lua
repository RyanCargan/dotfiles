-- Generated by hyprlang2lua, manually reviewed against hl.meta.lua stub.
-- Fix applied on top of the raw autogen output:
--   SUPER+SHIFT+S was bound twice in the original .conf (screenshot AND
--   special:magic move). Screenshot moved to SUPER+SHIFT+G, plus dedicated
--   Print / SUPER+Print binds added as alternates.
...
-- FIX: original bound SUPER+SHIFT+S twice (screenshot + special:magic move).
-- Screenshot moved to SUPER+SHIFT+G (distinct from F/G fullscreen combos
-- above since this uses SHIFT too), plus Print-key alternates.
